...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < count; i++) {
        if (myrank == 0)
            ...
            data[i] = i;   /* initialize all other data buffers */
    }

    /* Simulate MPI_Bcast using point-to-point communication */
    if (myrank == 0) {
        for (int i = 1; i < nprocs; i++) {  /* send to all other ranks */
    ...
    if (myrank != 0) {
        ...
        if (good_data == 0) {
            fprintf(stdout, "Whoa! The data is incorrect on rank %d\n", myrank);
        } else {
