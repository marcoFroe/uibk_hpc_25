...
int main(int argc, char** argv) {
	...
	if(argc == 3) {
		...
		printf("No arguments given, using default size (%d x %d).\n", global_sizeX, global_sizeY);
	}
	...
	if(rank == 0) {
		global_image = malloc((size_t)global_sizeX * global_sizeY * NUM_CHANNELS);
		mem_checker(global_image);
		global_cells = prepare_data(global_sizeX, global_sizeY);
	}

	const int total_elements = global_sizeX * global_sizeY;

	const int split_rem = total_elements % NUM_SPLITS;
	const int split_base = total_elements / NUM_SPLITS;
	...
	for(int split = 0; split < NUM_SPLITS; split++) {
		// compute split interval [start_idx, end_idx)
		int split_start = split * split_base + (split < split_rem ? split : split_rem);
		int split_size = split_base + (split < split_rem ? 1 : 0);

		DistInfo info;
		setupDistCalls(split_size, num_ranks, &info);

		// receive count per rank
		int local_count = info.recvcounts[rank];

		cell_t* local_cells = malloc((size_t)local_count * sizeof(cell_t));
		mem_checker(local_cells);

		cell_t* iteration_cells = NULL;
		if(rank == 0) {
			iteration_cells = malloc((size_t)split_size * sizeof(cell_t));
			mem_checker(iteration_cells);
			// copy the slice of global_cells into iteration_cells
			memcpy(iteration_cells, global_cells + split_start,
			       (size_t)split_size * sizeof(cell_t));
		}

		MPI_Scatterv(iteration_cells, info.recvcounts, info.displs, MPI_Cells_t, local_cells,
		             local_count, MPI_Cells_t, 0, MPI_COMM_WORLD);

		calcMandelbrot(local_cells, local_count, global_sizeX, global_sizeY);

		MPI_Gatherv(local_cells, local_count, MPI_Cells_t, iteration_cells, info.recvcounts,
		            info.displs, MPI_Cells_t, 0, MPI_COMM_WORLD);

		if(rank == 0) {
			memcpy(global_cells + split_start, iteration_cells,
			       (size_t)split_size * sizeof(cell_t));
			free(iteration_cells);
		}
...
void calcMandelbrot(cell_t* cells, int length, int global_X, int global_Y) {
	const float left = -2.5f, right = 1.0f;
	...
	for(int i = 0; i < length; i++) {
		...
		while((x * x + y * y <= 4.0f) && (numIterations < MAX_ITER)) {
		...
		double value = fabs((numIterations / (double)MAX_ITER)) * 200.0;
...
cell_t* prepare_data(int sizeX, int sizeY) {
	cell_t* cells = calloc((size_t)sizeX * sizeY, sizeof(cell_t));
	mem_checker(cells);
...
void create_mpi_type(MPI_Datatype* type) {
	...
	MPI_Aint displacements[5] = { offsetof(cell_t, x), offsetof(cell_t, y), offsetof(cell_t, red),
		                          offsetof(cell_t, green), offsetof(cell_t, blue) };
	MPI_Datatype types[5] = { MPI_INT, MPI_INT, MPI_UNSIGNED_CHAR, MPI_UNSIGNED_CHAR,
		                      MPI_UNSIGNED_CHAR };

	MPI_Type_create_struct(5, block_lengths, displacements, types, type);
