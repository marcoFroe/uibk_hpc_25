...
void calcMandelbrot(uint8_t* image, int sizeX, int sizeY) {
	...
	// Mandelbrot is symmetric on the real axis
	for(int pixelY = sizeY / 2; pixelY < sizeY; pixelY++) {
	...
	uint8_t* last_line = image + IND(sizeY - 1, 0, sizeY, sizeX, 0);
	for(int pixelY = 0; pixelY < sizeY / 2; pixelY++) {
		memcpy(image + IND(pixelY, 0, sizeY, sizeX, 0), last_line - IND(pixelY, 0, sizeY, sizeX, 0), sizeof(uint8_t) * NUM_CHANNELS * sizeX);
	}
