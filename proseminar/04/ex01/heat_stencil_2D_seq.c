...
int main(int argc, char** argv) {
    ...
    */
    ...
    for(int t = 0; t < T; t++) {
        ...
        for(int j = 0; j < N; j++) {
            if(0 == source_x && j == source_y) {
                    continue;
                }
        
            double up = current[IND(0, j)];
            double down = current[IND(1, j)];
            double left = (j > 0) ? current[IND(0, j - 1)] : current[IND(0, j)];
            double right = (j < N - 1) ? current[IND(0, j + 1)] : current[IND(0, j)];
            double here = current[IND(0, j)];
