...
int main(int argc, char** argv) {
		...
		for(int i = 0; i < N; i++) {
			for(int j = 0; j < N; j++) {
				if(i == source_x && j == source_y) {
					...
					// get components
					double up = (i > 0) ? A[IND(i - 1, j)] : A[IND(i, j)];
					double down = (i < N - 1) ? A[IND(i + 1, j)] : A[IND(i, j)];
					double left = (j > 0) ? A[IND(i, j - 1)] : A[IND(i, j)];
					double right = (j < N - 1) ? A[IND(i, j + 1)] : A[IND(i, j)];
					// calculate average
					B[IND(i, j)] = (up + down + left + right) / 4.0;
				}
			}
		}
	...
	// write output for automatic plot generation
	FILE* fptr = fopen("./results/serial_test_output.txt", "a");
	if(fptr == NULL) {
	...
	fprintf(fptr, "group: %d, num_threads: %d, time: %f\n", N, threads, end_time - start_time);
	fclose(fptr);
