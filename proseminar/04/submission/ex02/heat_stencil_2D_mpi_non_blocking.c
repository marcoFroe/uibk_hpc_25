...
int main(int argc, char** argv) {
	...
	for(int t = 0; t < T; t++) {
		MPI_Isend(current + (sec_size - 1) * N, N, MPI_DOUBLE, lower_rank, 42, MPI_COMM_WORLD,
		          rq_array);
		MPI_Isend(current, N, MPI_DOUBLE, upper_rank, 42, MPI_COMM_WORLD, rq_array + 2);
		MPI_Irecv(ghost_up, N, MPI_DOUBLE, upper_rank, 42, MPI_COMM_WORLD, rq_array + 1);
		MPI_Irecv(ghost_down, N, MPI_DOUBLE, lower_rank, 42, MPI_COMM_WORLD, rq_array + 3);
		...
		for(int i = 1; i < sec_size - 1; i++) {
			...
			next[IND(i, 0)] = here + 0.2 * (273 + current[IND(i + 1, 0)] + current[IND(i, 1)] +
			                                current[IND(i - 1, 0)] - 4 * here);
			for(int j = 0; j < N; j++) {
			...
			next[IND(i, N - 1)] =
			    here + 0.2 * (273 + current[IND(i + 1, N - 1)] + current[IND(i, N - 2)] +
			                  current[IND(i - 1, N - 1)] - 4 * here);
		}

		MPI_Waitall(NUM_RQ, rq_array, MPI_STATUS_IGNORE);
		// calculate first row on its own
		next[IND(0, 0)] =
		    here + 0.2 * (273 + ghost_up[0] + current[IND(1, 0)] + current[IND(0, 1)] - 4 * here);
		for(int j = 1; j < N - 1; j++) {
			double up = ghost_up[j];
		...
		next[IND(0, N - 1)] = here + 0.2 * (273 + +ghost_up[N - 1] + current[IND(1, N - 1)] +
		                                    current[IND(0, N - 2)] - 4 * here);

		// calculate final row on its own
		here = current[IND(sec_size - 1, 0)];
		next[IND(sec_size - 1, 0)] =
		    here + 0.2 * (273 + ghost_down[0] + current[IND(sec_size - 2, 0)] +
		                  current[IND(sec_size - 1, 1)] - 4 * here);
		for(int j = 1; j < N - 1; j++) {
			int i = sec_size - 1;

			double up = current[IND(i - 1, j)];
			double down = ghost_down[j];
			double left = current[IND(i, j - 1)];
			double right = current[IND(i, j + 1)];
			here = current[IND(i, j)];
		...
		here = current[IND(sec_size - 1, N - 1)];
		next[IND(sec_size - 1, N - 1)] =
		    here + 0.2 * (273 + ghost_down[N - 1] + current[IND(sec_size - 2, N - 1)] +
		                  current[IND(sec_size - 1, N - 2)] - 4 * here);
