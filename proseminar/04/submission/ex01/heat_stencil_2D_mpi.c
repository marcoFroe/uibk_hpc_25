...
int main(int argc, char** argv) {
	...
	for(int t = 0; t < T; t++) {
        ...
        next[IND(0, 0)] = here + 0.2 * (273 + ghost_up[0] + current[IND(1, 0)] + current[IND(0, 1)] - 4 * here);
        for(int j = 1; j < N - 1; j++) {        
        ...
        next[IND(0, N - 1)] = here + 0.2 * (273 + + ghost_up[N - 1] + current[IND(1, N - 1)] + current[IND(0, N - 2)] - 4 * here);
        ...
        next[IND(sec_size - 1, 0)] = here + 0.2 * (273 + ghost_down[0] + current[IND(sec_size - 2, 0)] + current[IND(sec_size - 1, 1)] - 4 * here);
        for(int j = 1; j < N - 1; j++) {
            int i = sec_size - 1;
        ...
        next[IND(sec_size - 1, N - 1)] = here + 0.2 * (273 + ghost_down[N - 1] + current[IND(sec_size - 2, N - 1)] + current[IND(sec_size - 1, N - 2)] - 4 * here);
